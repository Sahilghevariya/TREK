...
async function runBackup(): Promise<void> {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      archive.pipe(output);
      const { TABLES } = require('./db/database');
      const { getTableModel } = require('./models');
      Promise.all(TABLES.map(async (tableName: string) => [tableName, await getTableModel(tableName).find({}).lean()]))
        .then((entries) => {
          archive.append(JSON.stringify({ version: 1, tables: Object.fromEntries(entries) }, null, 2), { name: 'mongo-data.json' });
...
function startTodoReminders(): void {
  ...
  todoReminderTask = cron.schedule('0 9 * * *', async () => {
    try {
      ...
      // stored as YYYY-MM-DD text for predictable date comparisons.
