...
import fs from 'fs';
import { db, reinitialize, TABLES } from '../db/database';
...
export async function createBackup(): Promise<BackupInfo> {
  ...
  try {
    ...
    for (const tableName of TABLES) {
      mongoData[tableName] = await getTableModel(tableName).find({}).lean();
    }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      archive.append(JSON.stringify({ version: 1, tables: mongoData }, null, 2), { name: 'mongo-data.json' });
...
export async function restoreFromZip(zipPath: string): Promise<RestoreResult> {
  ...
  try {
    ...
    const extractedData = path.join(extractDir, 'mongo-data.json');
    if (!fs.existsSync(extractedData)) {
      fs.rmSync(extractDir, { recursive: true, force: true });
      return { success: false, error: 'Invalid backup: mongo-data.json not found', status: 400 };
    }

    let parsed: { tables?: Record<string, unknown[]> };
    try {
      parsed = JSON.parse(fs.readFileSync(extractedData, 'utf8'));
      ...
      fs.rmSync(extractDir, { recursive: true, force: true });
      return { success: false, error: 'Uploaded file is not a valid MongoDB TREK backup', status: 400 };
    }
