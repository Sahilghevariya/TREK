...
export function createApp(): express.Application {
  ...
  app.get('/api/addons', authenticate, (_req: Request, res: Response) => {
    const addonRows = db.prepare('SELECT id, name, type, icon, enabled FROM addons WHERE enabled = 1 ORDER BY sort_order').all() as Pick<Addon, 'id' | 'name' | 'type' | 'icon' | 'enabled'>[];
    const addons = [...new Map(addonRows.map(addon => [addon.id, addon])).values()];
