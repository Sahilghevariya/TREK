...
const TOP_NAV_ADDONS = ['vacay', 'atlas']
...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare }: NavbarProps): React.ReactElement {
  ...
  // Keep the top-level navigation stable and free of duplicate addon entries.
  const globalAddons = Array.from(
    new Map(
      allAddons
        .filter((a: Addon) => a.type === 'global' && a.enabled && TOP_NAV_ADDONS.includes(a.id))
        .map((addon: Addon) => [addon.id, addon])
    ).values()
  ).sort((a, b) => TOP_NAV_ADDONS.indexOf(a.id) - TOP_NAV_ADDONS.indexOf(b.id))
